...
def csv_rules(content):
    blocks = re.findall(r'(?m:^)<csv>\s*{(?s:(?:[^{}]+|\{.*?\})*)}', content)
    
    for m in blocks:
        match = re.match(r'(?m:^)<csv>\s*{((?s:(?:[^{}]+|\{.*?\})*))}', m)
        
        repl  = '_f = StringIO(_input)\n_reader = csv.reader(_f, delimiter=",")\n'
        repl += 'for _row in _reader:\n'
        
        repl2 = match.group(1)
        repl2 = re.sub(r'\$(\d+)', r'     _row[\1]', repl2)
        
        repl += repl2        
         
        content = re.sub(r'(?m:^)<csv>\s*{(?s:(?:[^{}]+|\{.*?\})*)}', repl, content, 1)
    return content
...
if __name__ == '__main__':
    ...
    with open(sys.argv[1]) as fp:
        ...
        file_content = csv_rules(file_content)
    ...
    _input = get_input()
    proc = subprocess.Popen(['python', '_script.py'],
